...
public class SetupPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        gbc.gridx = 0; gbc.gridy = 0; gbc.gridwidth = 2; // Chiếm trọn 2 cột
        add(titleLabel, gbc);
        ...
        gbc.gridy = 5; gbc.gridwidth = 2; // Chiếm 2 cột dưới cùng
        add(buttonPanel, gbc);
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int row = 0; row < getHeight() / size + 1; row++) {
            for (int col = 0; col < getWidth() / size + 1; col++) {
                if ((row + col) % 2 == 0) g2.setColor(new Color(30, 30, 30));
    ...
    private void selectColorButton(int type) {
        ...
        if (type == 0) btnWhite.setBorder(new LineBorder(Color.yellow, 3));
        if (type == 1) btnBlack.setBorder(new LineBorder(Color.yellow, 3));
        if (type == 2) btnRandom.setBorder(new LineBorder(Color.yellow, 3));
    }
    ...
    private void startGame() {
        ...
        if (p2.isEmpty()) p2 = isPvEMode ? "AI Cờ Vua" : "Player 2";

        // Xử lý logic Random màu
        int finalP1Color;
        if (selectedColorChoice == 2) {
            // Random ra 0 hoặc 1
            finalP1Color = Math.random() < 0.5 ? GamePanel.WHITE : GamePanel.BLACK;
        } else {
            // Nếu chọn Trắng (0) hoặc Đen (1)
            finalP1Color = (selectedColorChoice == 0) ? GamePanel.WHITE : GamePanel.BLACK;
        }
    ...
    class ColorButton extends JButton {
        ...
        protected void paintComponent(Graphics g) {
            ...
            if (type == 0) { 
                ...
                g2.fillRect(2, 2, 36, 36);
                // Vẽ thêm viền trắng nhỏ bên trong để nút đen không bị chìm vào nền đen của Panel
                g2.setColor(Color.white);
